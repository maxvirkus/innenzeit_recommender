...
2. PRÄZISIEREN: Ergibt das Gespräch konkrete neue Fakten (wenig Zeit, Abneigung gegen bestimmte Übungen/Kategorien, eine beim Check nicht genannte Stimmung), rufe verfeinere_empfehlung auf. Nur dann.
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!body?.messages?.length || !body.context?.selectedMoodIds?.length) {
